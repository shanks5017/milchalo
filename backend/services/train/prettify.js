...
class Prettify {
  decodeFares(fareString, classBinary) {
    if (!fareString || !classBinary) return [];
    
    const n_Classes = ['', '1A', '2A', '3A', 'CC', 'FC', 'SL', '2S', '3E', 'GN', 'EA', 'EC', 'EV', 'VC', 'VS'];
    const parts = fareString.split(":");
    if (parts.length < 3) return [];
    
    const fares = [];
    
    for (let i = 1; i < n_Classes.length; i++) {
      // classBinary is a 15-char string (e.g. 011001001000000)
      // Index 0 of classBinary maps to n_Classes[1] ('1A')
      if (classBinary[i - 1] === '1') {
        const clsName = n_Classes[i];
        // Fares for the classes start from parts[2] onwards.
        // So n_Classes[1] ('1A') is at parts[2].
        // This means parts[i + 1] contains the fare for n_Classes[i].
        const fareData = parts[i + 1];
        
        if (fareData && fareData.includes(",")) {
          const fareVals = fareData.split(",");
          const adultFare = parseInt(fareVals[0]) || 0;
          const tatkalExtra = parseInt(fareVals[3]) || 0;
          const tatkalAdult = adultFare + tatkalExtra;
          
          if (adultFare > 0) {
            fares.push({
              class: clsName,
              price: `₹${adultFare}`,
              tatkalPrice: `₹${tatkalAdult}`,
              status: "AVAILABLE" // Real availability requires live IRCTC parsing, defaulting to AVAILABLE for base routing
            });
          }
        }
      }
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data1.length === 2) {
          ...
          // Native Fare Decoding
          let farePart = [];
          if (i + 1 < data.length) {
            farePart = data[i + 1].split("~^")[0].split("~");
          }
          obj["running_classes"] = farePart[0] || "";
          obj["fare_string"] = farePart[20] || "";
          obj["fares"] = this.decodeFares(obj["fare_string"], obj["running_classes"]);
